...
vector<shared_ptr<LWECiphertextImpl>> adder_round_2_to_4(
  ...
  shared_ptr<LWECiphertextImpl> result_ct_0 =
      crypto_context.EvalBinGate(XOR_FAST, initial_bit_0, added_bit);
  ...
  shared_ptr<LWECiphertextImpl> result_ct_1 = crypto_context.EvalBinGate(
      XOR_FAST, carry_1, initial_bit_1);  // they can be both 1
...
vector<shared_ptr<LWECiphertextImpl>> adder_round_1(
  ...
  result_ct_0 =
      crypto_context.EvalBinGate(XOR_FAST, result_ct_0, bits_to_add[1]);
  ...
  result_ct_0 =
      crypto_context.EvalBinGate(XOR_FAST, result_ct_0, bits_to_add[2]);
  ...
  result_ct_0 =
      crypto_context.EvalBinGate(XOR_FAST, result_ct_0, bits_to_add[3]);
  shared_ptr<LWECiphertextImpl> carry_2 =
      crypto_context.EvalBinGate(AND, result_ct_1, carry_1);
  result_ct_1 = crypto_context.EvalBinGate(XOR_FAST, carry_1,
...
int main() {
  ...
  for (int i = 0; i < 12; i++) {
    secret_code.push_back(cc.Encrypt(sk, code_bits[i], FRESH));
  }
  ...
  while (attempts_made < MAX_ATTEMPTS && !game_is_won) {
    ...
    for (int i = 0; i < 12; i++) {
      challenger.push_back(cc.Encrypt(sk, challenge_bits[i], FRESH));
    }
    ...
    for (int i = 0; i < 12; i++) {
      ...
      xnor_red_results.push_back(
          cc.EvalBinGate(XNOR_FAST, secret_code[i], challenger[i]));
    }
    ...
    for (int j = 1; j < 4; j++) {
      for (int i = 0; i < 12; i++) {
        // the endianese of the next vectors can be ignored as position won't
        // matter at the end
        xnor_white_results.push_back(cc.EvalBinGate(
            XNOR_FAST, secret_code[(i + 3 * j) % 12], challenger[i]));
      }
    }
